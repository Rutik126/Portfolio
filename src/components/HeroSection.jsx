  ...
  return (
    ...
    >
      <div className="sticky top-0 h-screen overflow-hidden">
        ...
        <p
          ref={introRef}
          className="absolute left-[5%] top-[58%] z-30 max-w-[420px] -translate-y-1/2 text-left text-[clamp(16px,1.2vw,20px)] font-medium tracking-[-0.01em] text-[#666666]"
        >
        ...
        <div
          ref={portraitRef}
          className="pointer-events-none absolute top-[18%] left-1/2 z-20 h-[62%] w-auto -translate-x-1/2 md:h-[85%]"
        >
          <img
            src={portraitImage}
        ...
        <div
          ref={capabilityRef}
          className="absolute inset-x-0 bottom-[4%] z-30 px-4 sm:bottom-[4%] md:bottom-[3%]"
        >
          <div className="group mx-auto flex w-fit max-w-full items-end justify-center -space-x-[44px] transition-all duration-500 hover:space-x-2 md:-space-x-[52px] md:hover:space-x-4">
            {heroContent.capabilityCards.map((card, index) => {
              ...
              return (
                <article
                  key={card.label}
                  className={`pointer-events-auto relative flex min-h-[94px] w-[108px] flex-col justify-between overflow-hidden rounded-[24px] border border-black/10 px-3 py-3 shadow-[0_18px_34px_rgba(15,15,15,0.08)] transition-all duration-500 hover:z-40 hover:-translate-y-2 hover:rotate-0 md:min-h-[108px] md:w-[128px] md:px-4 md:py-4 md:group-hover:translate-y-0 md:group-hover:rotate-0 ${capabilityCardStyles[index]}`}
        ...
        <div
          ref={actionsRef}
          className="absolute left-[5%] top-[60%] z-30 flex max-w-[430px] -translate-y-1/2 pr-4 sm:top-[72%]"
        >
