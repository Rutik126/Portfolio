...
const ArticlesSection = memo(function ArticlesSection() {
    ...
    cardRefs.current.forEach((card, index) => {
      const state =
        index === activeIndex
          ...
          : index < activeIndex
            ? { scale: 0.96, opacity: 0.56, filter: 'blur(8px)', y: -20 }
            : { scale: 0.98, opacity: 0.82, filter: 'blur(0px)', y: 16 }
  ...
  return (
    <section id="articles" className="relative z-50 -mt-[24vh] rounded-t-[24px] bg-[#090a11] px-6 py-24 md:px-8 xl:px-16">
      <div className="mx-auto max-w-[1200px]">
        ...
        <div className="space-y-10">
          {articles.map((article, index) => (
            <div key={article.title} className="sticky top-24">
              <div
                ref={(element) => {
              ...
              <motion.div whileHover={{ y: -6 }} transition={{ duration: 0.25 }}>
                <Card
                  ...
                  className="relative grid min-h-[340px] grid-cols-1 gap-10 overflow-hidden md:grid-cols-[1.1fr_0.9fr]"
                >
